...
This topic describes how to install the Application Service Adapter for VMware Tanzu Application Platform:
...
1. Create a `tas-adapter-values.yml` file with the desired installation settings, following the schema specified for the package.
    ...
    - `APP-TLS-KEY` private key for applications deployed using the Application Service Adapter.
...
1. Verify that the package install was successful:
   ...
   The output looks like the following:
    ...
    ```
## <a id="configure-tbs"></a>Configure Tanzu Build Service to work with the Application Service Adapter

In order to stage applications, you must create the following secret in the `cf` namespace created by the Application Service Adapter install package.

To configure Tanzu Build Service to work with the Application Service Adapter:

1. Create the image registry secret for Tanzu Build Service:
