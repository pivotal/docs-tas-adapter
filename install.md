...
1. Include the following values in your `tas-adapter-values.yml` file:
   ...
   - `ADMIN-USERNAME` is the name of an existing user in the Kubernetes cluster
    ...
    It is important to note that, these usernames must be identities which can be recognized as users in subject section of Kubernetes RBAC resources, such as `RoleBindings`. They can differ and might not match the user entries in your local Kubeconfig file. During the initial installation, you can leave this entry blank if you're unsure of the username. After installation, login with the cf CLI and run the `cf curl /whoami` command to confirm the username. Then, update the installation with the correct name value. For more information about user subject names in Kubernetes, see the [Referring to subjects](https://kubernetes.io/docs/reference/access-authn-authz/rbac/#referring-to-subjects) section of _Using RBAC Authorization_ and the [Authenticating](https://kubernetes.io/docs/reference/access-authn-authz/authentication/) topic in the Kubernetes project documentation.
