...
the [kapp cli]() as the most convenient method to gather the logs from the pods
in one or more deployments. For example:
...
provide [standard Prometheus performance metrics](https://book.kubebuilder.io/reference/metrics-reference.html) generated by the kubebuilder
controller runtime.

Application Service Adapter controller managers are deployed with annotations
that should make them discoverable by a Prometheus server deployed to the same
cluster.
