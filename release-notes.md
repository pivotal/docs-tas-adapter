...
* DockerHub is currently not supported as an application container registry. When `https://index.docker.io/v1` is configured as the registry hostname, `cf push` will fail with the following error: `"Error uploading source package to the container registry"`.
