...
For managed clusters that come from public-cloud IaaSes, you generally don't
need to be concerned about scaling the control plane, but if you are managing
your own Kubernetes clusters, you will need to ensure that you have provisioned
adequate capacity for the `apiserver` and `etcd` components. See the [Kubernetes
...
vertically may improve performance.
...
### Horizontal Scaling
Controller managers use leader-election to designate a single active instance,
so horizontal scaling is not an effective strategy to improve performance of a
running deployment. You may choose to scale out your deployments to improve
availability however. Multi-instance deployments will allow rolling deployments
so that controllers and web-hooks don't become unresponsive during upgrades.
Horizontal scale can be achieved using the `replicas` scaling configuration.
...
By default, `korifi-api-deployment` runs with 2 replicas, so it should already
