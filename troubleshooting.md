...
This section contains generic techniques that can be used to gather information about the status of the Application Service Adapter to aid in troubleshooting. In general the latest version of the Application Service Adapter and the CF CLI should be used.
...
To stream logs from all components of an Application Service Adapter installation:
    ...
    ```bash
    kapp logs --app tas-adapter --follow
    ```

To stream logs from a specific deployment:

    ```bash
    kapp logs --app tas-adapter --pod-name DEPLOYMENT-NAME% --follow
    ```
...
1. The API Deployment is named `korifi-api-deployment` and is tasked with responding to API requests sent to the Application Service Adapter. This Deployment's logs are useful to debug failures that do not make it to the controller manager. Logs for failures related to the image registry may also show up in this Deployment's logs.
1. The controller manager Deployment is named `korifi-controllers-controller-manager` and is tasked with processing commands for the Application Service Adapter. This Deployment's logs is an excellent starting point when debugging failures as it is where most commands flow through before being processed by more specific components (The exception being API commands that fail before getting to the controller manager).
1. The telemetry informer Deployment is named `tas-adapter-telemetry-informer` and is tasked with handling all outgoing telemetry.
1. The kpack build controller manager Deployment is named `korifi-kpack-build-controller-manager` and is tasked with building Images when using the default builder/runner flow.
1. The statefulset runner controller manager Deployment is named `korifi-statefulset-runner-controller-manager` and is tasked with generating StatefulSets for apps when using the default builder/runner flow.
1. The job task runner controller manager Deployment is named `korifi-job-task-runner-controller-manager` and is tasked with handling all task related functionality.
1. The cartographer builder runner controller manager Deployment is named `cartographer-builder-runner-controller-manager` and is tasked with creating Workloads for apps when using the experimental cartographer builder/runner flow.
...
To stream logs from all components from a given TAP application:

    ```bash
    kapp logs --app APP-NAME --follow -n tap-install
    ```
    Where APP-NAME is the name of the TAP application (e.g.
    "buildservice-ctrl").
...
1. The kpack controller Deployment is part of the `buildservice-ctrl` application and is tasked with processing any kpack commands. If any failures in the build image process occurs in kpack itself, this Deployment's logs can provide further information.
1. The contour Deployment is part of the `contour-ctrl` application and is tasked with creating an ingress into the system. If a failure to connect to the Application Service Adapter occurs, this Deployment's logs can provide further information.
1. The cartographer controller Deployment is part of the `cartographer-ctrl` application and is tasked with processing cartographer ClusterSupplyChains when using the experimental cartographer builder/runner flow. If a failure to create an Image, Build, ConfigMap, or StatefulSet occurs, this Deployment's logs can provide further information.
...
In addition to the events present on the individual Kubernetes objects themselves (Like pods), there are Kubernetes system events that can provide additional debug information in cases where system restrictions are preventing objects from being created. This can be things like not enough resources or pods being evicted. The logs for the system can be queried with the following command:
...
|--------------------|--------------|----------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ClusterBuilder     | kpack        | Cluster              | ClusterBuilders are kpack components installed by TAP that are used to build images. If they are not present, image builds may not complete successfully.                                                                                                                                                                                                    |
...
| ServiceAccount     | kubernetes   | `cf`                 | ServiceAccounts are created to manage CF permissions. Some ServiceAccounts are designed to be propagated to CFOrg/CFSpace namespaces. These objects will be designated with the `cloudfoundry.org/propagate-cf-role: "true"` annotation.                                                                                                                     |
...
|-----------------|-----------|--------------------|------------------------------|----------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| CFOrg           | CF        | `cf`               | `korifi-api-deployment`      | `korifi-controllers-controller-manager`            | CFOrgs are objects are created when the `cf create-org` command is run. A corresponding namespace should also be created by the `korifi-controllers-controller-manager`. |
| CFSpace         | CF        | CFOrg Namespace    | `korifi-api-deployment`      | `korifi-controllers-controller-manager`            | CFSpaces are created when the `cf create-space` command is run. A corresponding namespace should also be created by the `korifi-controllers-controller-manager`.         |
...
|-----------------------------|-----------|-----------------------------------------|----------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| CFApp                       | CF        | `korifi-api-deployment`                 | `korifi-controllers-controller-manager`      | The 1st object in the image build chain that's created when the `cf push` command is run.                                                                                                                                                                                                                                                  |
| CFPackage                   | CF        | `korifi-api-deployment`                 | `korifi-controllers-controller-manager`      | The 2nd object in the image build chain that's created when the `cf push` command is run.                                                                                                                                                                                                                                                  |
| CFBuild                     | CF        | `korifi-api-deployment`                 | `korifi-controllers-controller-manager`      | The 3rd object in the image build chain that's created when the `cf push` command is run.                                                                                                                                                                                                                                                  |
| BuildWorkload               | CF        | `korifi-controllers-controller-manager` | `korifi-kpack-build-controller-manager`      | The 4th object in the image build chain that's created when the `cf push` command is run.                                                                                                                                                                                                                                                  |
...
| Kind                        | Component  | Created By                                     | Reconciled By                                                                                                                                                         | Notes                                                                                                                                                                                                                      |
|-----------------------------|------------|------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| CFProcess                   | CF         | `korifi-api-deployment`                        | `korifi-controllers-controller-manager`                                                                                                                               | The 1st object in the app run chain that's created when the `cf push` command is run.                                                                                                                                      |
| AppWorkload                 | CF         | `korifi-controllers-controller-manager`        | `korifi-statefulset-runner-controller-manager`                                                                                                                        | The 2nd object in the app run chain that's created when the `cf push` command is run.                                                                                                                                      |
| StatefulSet                 | Kubernetes | `korifi-statefulset-runner-controller-manager` | NA                                                                                                                                                                    | The 3rd object in the app run chain that's created when the `cf push` command is run. There should be a StatefulSet for each CFProcess.                                                                                    |
| Pod                         | Kubernetes | Kubernetes                                     | The App Process Pods are last in the app run chain and created by Kubernetes based on the StatefulSet. The number of pods are based on the StatefulSet replica count. |
...
| CFPackage        | CF            | `korifi-api-deployment`                          | `korifi-controllers-controller-manager`          | The 2nd object in the image build chain that's created when the `cf push` command is run.                                                                                                                                                                                                                                                  |
| CFBuild          | CF            | `korifi-api-deployment`                          | `korifi-controllers-controller-manager`          | The 3rd object in the image build chain that's created when the `cf push` command is run.                                                                                                                                                                                                                                                  |
| BuildWorkload    | CF            | `korifi-controllers-controller-manager`          | `cartographer-builder-runner-controller-manager` | The 4th object in the image build chain that's created when the `cf push` command is run.                                                                                                                                                                                                                                                  |
| Workload         | Cartographer  | `cartographer-builder-runner-controller-manager` | `cartographer-ctrl`                              | The 5th object in the image build chain that's created when the `cf push` command is run.                                                                                                                                                                                                                                                  |
...
|-------------|------------|-----------------------------------------|--------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| CFProcess   | CF         | `korifi-api-deployment`                 | `korifi-controllers-controller-manager`          | The 1st object in the app run chain that's created when the `cf push` command is run.                                                                                                                                                       |
| AppWorkload | CF         | `korifi-controllers-controller-manager` | `cartographer-builder-runner-controller-manager` | The 2nd object in the app run chain that's created when the `cf push` command is run. `cartographer-builder-runner-controller-manager` edits the Workload created during the image build process with the information from the AppWorkload. |
| ConfigMap   | Kubernetes | `cartographer-ctrl`                     | NA                                               | The 3rd object in the app run chain that's created when the `cf push` command is run. A ConfigMap with the StatefulSet definition is created that spawns a StatefulSet by means of kapp.                                                    |
| StatefulSet | Kubernetes | `cartographer-ctrl`                     | NA                                               | The 4th object in the app run chain that's created when the `cf push` command is run. There should be a StatefulSet for each CFProcess.                                                                                                     |
| Pod         | Kubernetes | Kubernetes                              | NA                                               | The App Process Pods are last in the app run chain and created by Kubernetes based on the StatefulSet. The number of pods are based on the StatefulSet replica count.                                                                       |
| CFRoute     | CF         | `korifi-api-deployment`                 | `korifi-controllers-controller-manager`          | CFRoutes are created to be able to reach running apps. They can be created as part of the `cf push` command if a default or random route is specified, defined in a manifest, or as part of the `cf create-route` command.                  |
...
#### Troubleshooting Steps/Potential Solutions
1. Check the status of the ClusterBuilders by running `kubectl get clusterbuilder`. The `Ready` column should result in `True`.
...
#### Troubleshooting Steps/Potential Solutions
Creating the SCC will fail on a non-OpenShift cluster because the SCC CRD is only present on OpenShift. Reinstall Application Service Adapter with the OpenShift setting turned off.
...
#### Troubleshooting Steps/Potential Solutions
Without the SCC/ClusterRole/ClusterRoleBinding/RoleBinding, the Application Service Adapter deployments will not be able to deploy Pods. 
